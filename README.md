...
3. Separation of concerns - 
   ...
   * The REST controller passes CreateUserProfileRequest to the service.  The service validates the request and uses several data mappers to create the database entity.  Again, the calling program does not have to deal with any database schema changes.
4. Optimistic locking - the database verifies another thread has not modified the database row.  This required addition of a new column which resulted in no changes to the REST API.
5. Spring Retry and Recover - Spring retries the method several times and then calls the recover method.
   * The method also immediately calls a different recover method when a validation error occurs.  There is no reason to retry a method that will fail every time.
